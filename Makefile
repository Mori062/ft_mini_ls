...
define progress
    ...
    @if [ $(CURRENT_FILE) -gt $(TOTAL_FILES) ]; then \
        printf "$(GENERATE) $(YELLOW)Finish Generating FT_MINI_LS Object files !%-50.50s\n$(RESET)"; \
    fi
endef
...
$(NAME): $(OBJS)
	@ $(MAKE) -C ./libft
	@ $(CC) $(CFLAGS) -o $@ $^ $(LIBFT)
	@ printf "$(CHECK) $(BLUE)Compiling FT_MINI_LS...%-50.50s\n$(RESET)"
...
clean :
	@ $(MAKE) -C ./libft clean
	@ $(RM) $(OBJDIR)
	@ echo "$(REMOVE) $(BLUE)Remove FT_MINI_LS object files. $(RESET)"

fclean :
	@ $(MAKE) -C ./libft fclean
	@ $(RM) $(OBJDIR) $(NAME)
	@ echo "$(REMOVE) $(BLUE)Remove FT_MINI_LS object files and $(NAME). $(RESET)"
